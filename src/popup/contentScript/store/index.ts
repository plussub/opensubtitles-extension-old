...
export const useStore = defineStore('contentScript', {
  ...
  actions: {
    initialize() {
      ...
      this.requestAllContentScriptsToRegister();
        ...
        .forEach(({ contentWindow, requestId }) => {
          ...
          this.inputObservable
            .pipe(
              ...
              take(1),
              tap(({ origin, source, data: { id } }) => this.connectionSubject.next({
                action: 'ADD',
                origin,
                source: source as Window,
                id
